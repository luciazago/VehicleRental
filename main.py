...
def create_vehicle(): #This function asks the user for vehicle information, creates the correct type of vehicle, and returns the created object
    print("\nVehicle Type:")
    print("1. Car")
    print("2. Motorbike")
    print("3. Truck")
    ...
    if choice == "1":
        ...
        return Motorbike(brand, color, plate, model, matriculation_date, mileage)
    elif choice == "3":
        return Truck(brand, color, plate, model, matriculation_date, mileage)
    else:
        print("Invalid vehicle type.")
        return None

def create_client(): #This function asks the user for the client’s information, creates a Client object, and returns it
...
def create_worker(): #This function asks the user for the worker’s information, creates an Admin object, and returns it
...
def create_rental(manager): #This function asks the user for rental information and uses the manager to create a new rental.
...
def view_client_vehicles(manager): #This function asks for a client ID, gets that client from the manager, and shows all vehicles assigned to that client.
    client_id = input("Client ID: ")
    client = manager.get_client(client_id) #Uses the manager to find the client with that ID and saves it in client
    vehicles = client.vehicles #Gets the list of vehicles assigned to that client.
