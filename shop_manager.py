...
class ShopManager:
        ...
        for rental in self.__rentals:
            if (
                ...
                raise VehicleAlreadyRentedError(
                    f"Cannot remove vehicle '{plate}' because it has an active or upcoming rental." )
        for client in self.__clients:
            try:
    ...
    def remove_client(self, user_id):
        ...
        for rental in self.__rentals:
            if (
                ...
                raise VehicleAlreadyRentedError(
                    f"Cannot remove client '{user_id}' because they have an active or upcoming rental."
                )
    ...
    def add_worker(self, worker):
        if self._user_id_exists(worker.user_id):
            raise DuplicateIDError(
                f"A user with ID '{worker.user_id}' already exists."
            )
    ...
    def modify_rental(
        ...
        if new_end_date is not None:
            for other in self.__rentals:
                ...
                if same_vehicle and overlaps:
                    raise VehicleAlreadyRentedError(
                        "This modification would overlap with another rental."
                    )
