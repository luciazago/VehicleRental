...
class Vehicle(ABC):
    ...
    def validate_license_plate(self, plate):
        ...
        if not re.fullmatch(r"\d{4}[A-Z]{3}", plate):
            raise InvalidLicensePlateError(
                "License plate must have 4 digits followed by 3 letters."
            )
    ...
    def validate_date(self, matriculation_date):
        if not isinstance(matriculation_date, date):
            raise InvalidDateError(
                "Matriculation date must be a date object."
            )

        if matriculation_date > date.today():
            raise InvalidDateError(
                "Matriculation date cannot be in the future."
            )
    ...
    def update_brand(self, new_brand):
        self.__brand = self.validate_text_field(
            new_brand,
            "Brand"
        )

    def update_color(self, new_color):
        self.__color = self.validate_text_field(
            new_color,
            "Color"
        )

    def update_model(self, new_model):
        self.__model = self.validate_text_field(
            new_model,
            "Model"
        )
    ...
    def years_since_matriculation(self):
        return relativedelta(
            date.today(),
            self.__matriculation_date
        ).years

    @abstractmethod
    def next_itv(self):  # pragma: no cover
        pass

    @abstractmethod
    def next_maintenance(self):  # pragma: no cover
        pass
